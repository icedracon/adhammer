...
/// PKINIT with an optional CA-issued certificate (ESC1/cert-based). When `cert_der` is `None`
/// this is key-trust PKINIT (self-signed cert, for Shadow Credentials); when `Some`, the issued
/// cert is embedded in the CMS and its real issuer/serial identify the signer — the KDC then
/// validates the chain against its NTAuth store.
pub async fn pkinit_with_cert(
    ...
    // Key-trust (None) → self-signed cert; ESC1 (Some) → the CA-issued cert, whose real
    // issuer+serial identify the signer so the KDC can chain it to a trusted CA.
