//! Registry-only AD CS ESC decision rules — the "what does this raw registry value mean?"
//! layer, decoupled from the transport that reads it. Callers hand in registry values
//! (over MS-RRP, offline hive parse, or a mocked fixture); this module returns
//! [`EscHit`]s and turns them into [`adhammer_core::Finding`]s.
//!
//! Covers the ESC classes LDAP can't see:
//!
//! - **ESC6**  — CA `EditFlags` has `EDITF_ATTRIBUTESUBJECTALTNAME2` → requester-supplied SAN on
//!   *any* template.
//! - **ESC7**  — CA `Security` descriptor grants ManageCA / ManageCertificates to a non-Tier-0
//!   principal.
//! - **ESC10** — DC `Kdc\StrongCertificateBindingEnforcement` < 2 → weak Kerberos cert mapping.
...
//!
//! The wire probe that actually reads these values over MS-RRP lives in the `adhammer` binary
//! (`cli/src/esc_registry.rs`). It calls the functions here.
...
mod tests {
    ...
    fn esc6_fires_only_on_the_bit() {
        ...
        assert!(esc6(0x0014_0014).is_some());
        assert!(esc6(0x0000_0000).is_none());
        assert!(esc6(0x0002_0000).is_none());
    }

    #[test]
    fn esc11_fires_when_encryption_not_enforced() {
        assert!(esc11(0x0000_0000).is_some());
        assert!(esc11(0x0000_0040).is_some());
        assert!(esc11(0x0000_0200).is_none());
