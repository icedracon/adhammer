...
mod build_tests {
    ...
    /// Regression: a truncated object-ACE (ObjectType flag set, no GUID bytes) must not panic —
    /// it previously indexed `b[p..p+16]` out of bounds. Malformed SDs are attacker-influenced.
    #[test]
    fn truncated_object_ace_does_not_panic() {
        let mut sd = vec![1, 0, 0, 0]; // revision/sbz/control
        ...
        sd.extend_from_slice(&[0x05, 0, 0x20, 0]); // ACE: AccessAllowedObject, size 0x20
        sd.extend_from_slice(&0u32.to_le_bytes()); // mask
        sd.extend_from_slice(&1u32.to_le_bytes()); // obj_flags = ObjectType present …but no GUID follows
        assert!(parse(&sd).is_ok() || parse(&sd).is_err()); // just: no panic
    }

    /// Fuzz-lite: random + seed-mutated bytes into `parse` must never panic (deterministic seed).
    #[test]
