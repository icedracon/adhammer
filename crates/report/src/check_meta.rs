...
//! Finding.title / Finding.impact. This is the parallel copy for the clean case.
...
/// Look up a check's title/hypothetical impact/remediation/MITRE by its `Check::id()`.
/// Returns `CheckMeta::UNKNOWN` (all-empty) if the id is not in this table — safe default
/// that renders as "no description available" rather than crashing.
