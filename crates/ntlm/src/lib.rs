...
pub fn nt_owf_v2(password: &str, user: &str, domain: &str) -> [u8; 16] {
    nt_owf_v2_from_hash(&nt_hash(password), user, domain)
}

/// NTOWFv2 straight from an existing NT hash — the pass-the-hash entry point.
pub fn nt_owf_v2_from_hash(nt: &[u8; 16], user: &str, domain: &str) -> [u8; 16] {
...
impl Ntlm {
    ...
    /// Pass-the-hash: authenticate with a raw NT hash instead of a plaintext password.
    pub fn authenticate_hash(
        &self,
        challenge: &[u8],
        domain: &str,
        user: &str,
        ...
        self.authenticate_key(
            ...
            domain,
            user,
            &nt_owf_v2_from_hash(nt_hash, user, domain),
            workstation,
        )
    }
    ...
    fn authenticate_key(
        ...
        let (nt_response, session_base_key, lm_response) = ntlmv2_response(
            &resp_key,
            &ch.server_challenge,
            &client_challenge,
            timestamp,
...
mod tests {
    ...
    fn type3_roundtrips_identity() {
        ...
        let ntlm = Ntlm::new();
        let (t3, exported) = ntlm
            .authenticate(&type2, "Domain", "User", "Password", "WKS")
            .unwrap();
    ...
    fn capture_roundtrip_netntlmv2() {
        ...
        let ntlm = Ntlm::new();
        let (type3, _key) = ntlm
            .authenticate(&chal, "CORP", "victim", "S3cret!", "WS01")
            .unwrap();
    ...
    fn seal_pdu_roundtrip_against_server_direction() {
        ...
        for (seq, stub) in [&b"aaaabbbb"[..], b"ccccdddd", b"eeee"]
            .into_iter()
            .enumerate()
        {
    ...
    fn unseal_pdu_verifies_and_rejects_tampering() {
        ...
        let out = SealState::new(&exported)
            .unseal_pdu(&pdu, 24, stub.len(), &sig)
            .unwrap();
        assert_eq!(&out[..], stub);
        assert!(SealState::new(&exported)
            .unseal_pdu(&pdu, 24, stub.len(), &[0u8; 16])
            .is_err());
    }
}
