...
pub fn decode_lookup_names(stub: &[u8]) -> Result<Option<Sid>> {
    ...
    if dom_ref != 0 {
        ...
        if domains_ref != 0 {
            ...
            for (name_ref, sid_ref) in fixed {
                ...
                domain_sids.push(if sid_ref != 0 {
                    Some(decode_rpc_sid(&mut d)?)
                } else {
                    None
                });
            }
    ...
    Ok(Some(Sid {
        ...
        identifier_authority: dom.identifier_authority,
        sub_authorities: sub,
    }))
}
...
impl<'a> LsatClient<'a> {
    ...
    pub async fn open_policy(&mut self) -> Result<SamrHandle> {
        let resp = self
            .pipe
            .call(opnum::OPEN_POLICY2, &encode_open_policy2())
            .await?;
        let mut d = NdrDecoder::new(&resp);
        SamrHandle::decode(&mut d)
    }

    /// Resolve a name (e.g. "Administrator" or "CORP\\svc_sql") to its SID.
    pub async fn lookup_name(&mut self, policy: &SamrHandle, name: &str) -> Result<Option<Sid>> {
        let resp = self
            .pipe
            .call(opnum::LOOKUP_NAMES, &encode_lookup_names(policy, name))
            .await?;
